...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = (env.UNMASKBOT_PROFILE ?? env.CLAWDBOT_PROFILE)?.trim().toLowerCase();
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - Unmask Dev Workspace\n\nDefault dev workspace for unmaskbot gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",

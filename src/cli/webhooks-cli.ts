...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.unmask.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Unmaskbot hooks")
    ...
    .option("--hook-url <url>", "Unmaskbot hook URL")
    .option("--hook-token <token>", "Unmaskbot hook token")

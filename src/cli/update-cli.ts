...
const STEP_LABELS: Record<string, string> = {
  ...
  "unmaskbot doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "New version installed. Restart when ready.",
  "Fresh build, same focus.",
  "Upgrade done. Keep shipping.",
  "Patched and ready.",
  "Updated. Onward.",
];

const MAX_LOG_CHARS = 8000;
const DEFAULT_PACKAGE_NAME = "unmaskbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "moltbot"]);
const CLI_NAME = resolveCliName();
const CLAWDBOT_REPO_URL = "https://github.com/tryunmask/unmaskbot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "unmaskbot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("unmaskbot@")) return trimmed.slice("unmaskbot@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.UNMASKBOT_GIT_DIR?.trim() || process.env.CLAWDBOT_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `UNMASKBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set UNMASKBOT_GIT_DIR to an empty folder or an unmaskbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Unmaskbot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Unmaskbot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Unmaskbot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("unmaskbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("unmaskbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g unmaskbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g unmaskbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("unmaskbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("unmaskbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("unmaskbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("unmaskbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `unmaskbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `UNMASKBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set UNMASKBOT_GIT_DIR to an empty folder or an unmaskbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Unmaskbot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["unmaskbot update", "Update a source checkout (git)"],
        ["unmaskbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["unmaskbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["unmaskbot update --tag beta", "One-off update to a dist-tag or version"],
        ["unmaskbot update --no-restart", "Update without restarting the service"],
        ["unmaskbot update --json", "Output result as JSON"],
        ["unmaskbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["unmaskbot update wizard", "Interactive update wizard"],
        ["unmaskbot --update", "Shorthand for unmaskbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run unmaskbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.unmask.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["unmaskbot update status", "Show channel + version status."],
          ["unmaskbot update status --json", "JSON output."],
          ["unmaskbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.unmask.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {

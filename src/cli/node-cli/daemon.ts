...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("unmaskbot node install"),
    formatCliCommand("unmaskbot node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("unmaskbot node install --force")}`);
    }
    return;
  }

...
${formatHelpExamples([
  ['unmaskbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['unmaskbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'unmaskbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'unmaskbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['unmaskbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'unmaskbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.unmask.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.unmask.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['unmaskbot agents set-identity --agent main --name "Unmask" --emoji "🎭"', "Set name + emoji."],
  ["unmaskbot agents set-identity --agent main --avatar avatars/unmask.png", "Set avatar path."],
  ["unmaskbot agents set-identity --workspace ~/unmask --from-identity", "Load from IDENTITY.md."],
  [
    "unmaskbot agents set-identity --identity-file ~/unmask/IDENTITY.md --agent main",

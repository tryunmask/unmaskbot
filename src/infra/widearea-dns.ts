...
export const WIDE_AREA_DISCOVERY_DOMAIN = "unmaskbot.internal.";
export const WIDE_AREA_ZONE_FILENAME = "unmaskbot.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*(?:moltbot|unmaskbot)-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "unmaskbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "unmaskbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "unmaskbot-gw");
  ...
  records.push(`_unmaskbot-gw._tcp IN PTR ${instanceLabel}._unmaskbot-gw._tcp`);
  records.push(`${instanceLabel}._unmaskbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._unmaskbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; unmaskbot-content-hash: ${contentHash}\n${contentBody}`;
}
